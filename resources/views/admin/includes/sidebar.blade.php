...
{{-- <nav id="sideNav"
    ...
    <nav id="sideNav"
        class="fixed top-14 md:top-24 bottom-0 left-0 z-50 w-60 bg-darkest border-t-2 border-white transform transition-transform duration-300 ease-in-out -translate-x-full md:translate-x-0 md:block flex flex-col overflow-hidden">
            ...
            <ul>
                ...
                <li><a class="flex items-center gap-2 px-3 py-3 mx-5 my-2 text-brown font-medium hover:bg-primary hover:rounded-md hover:text-white {{ Route::is($item['sub_route']) || Route::is($item['route']) ? 'bg-primary rounded-md text-white' : '' }}"
                        ...
                        <span>{{ $item['label'] }}</span> <span
                            class="bg-[#00A8FF] text-white px-1 ml-5 text-sm rounded-sm">{{ $item['route'] ===
                            'buy-requests.index'
                            ? ($buyRequestsCount > 0
        ...
        {{-- <div class="mt-auto pb-20 w-48 mx-5 border-t border-brown md:absolute md:bottom-24 md:pb-0"> --}}
            <div class="w-full px-5 py-4 border-t border-brown bg-darkest shrink-0">
