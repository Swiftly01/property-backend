    ...
    @php
        $menuItems = [
            ['label' => 'Dashboard', 'icon' => 'dashboard.png', 'route' => 'index', 'sub_route' => 'index.*'],
            ['label' => 'Staffs', 'icon' => 'staff.svg', 'route' => 'test',  'sub_route' => 'index.*'],
            ['label' => 'Properties', 'icon' => 'property.svg', 'route' => 'properties.index',  'sub_route' => 'properties.*'],
            ['label' => 'Buy request', 'icon' => 'buy-request.svg', 'route' => 'buy-requests.index',  'sub_route' => 'buy-requests.*'],
            ['label' => 'Sell request', 'icon' => 'sell-request.svg', 'route' => 'sell-requests.index',  'sub_route' => 'sell-requests.*'],
            ['label' => 'Photograph', 'icon' => 'photograph.svg', 'route' => 'photographs.index', 'sub_route' => 'photographs.*'],
            ...
            ['label' => 'Settings', 'icon' => 'settings.png', 'route' => 'test',  'sub_route' => 'index.*'],
        ];
    @endphp
            ...
            @foreach ($menuItems as $index => $item)
                <li><a class="flex items-center gap-2 px-3 py-3 mx-5 my-2 text-brown font-medium  hover:bg-primary hover:rounded-md hover:text-white   {{  Route::is($item['sub_route']) || Route::is($item['route']) ? 'bg-primary rounded-md text-white' : ''  }}"
                        href="{{ route($item['route']) }}"> <img src="{{ asset('assets/icons/' . $item['icon']) }}"
                            alt="dashboard">
                        <span>{{ $item['label'] }}</span> <span class="bg-[#00A8FF] text-white  px-1 ml-5 text-sm rounded-sm">{{$item['route'] === 'buy-requests.index' ? '08' : ''  }}</span></a></li>
            @endforeach
        </ul>
