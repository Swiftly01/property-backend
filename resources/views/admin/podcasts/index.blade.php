...
@section('content')
    ...
    <div class="p-4 sm:p-6 lg:p-10">
        ...
        <div class="flex flex-wrap items-center justify-between mt-16 md:mt-0">
            <h1 class="font-bold text-gray-900 md:text-2xl">All podcast</h1>
            ...
            <a class="flex    bg-[#022A66] text-white px-2 py-1  md:px-4 md:py-2 rounded hover:bg-primary hover:scale-[1.02] transition-duration-200 active:scale-100"
                ...
                <span class="sm:font-normal md:font-medium">Add podcast</span>
            </a>
        ...
        <div class="mt-5 bg-white rounded lg:mt-14">
            <div class="flex flex-wrap items-center justify-between px-8 py-5">
                <h3 class="font-bold">All podcast</h3>

                <form id="searchForm" action="{{ route('podcasts.index') }}" method="GET">
                    ...
                    <tbody>
                        @foreach ($podcasts as $podcast)
                            <tr
                                ...
                                <td class="px-6 py-4">
                                    {{ $podcast->created_at->format('Y/m/d - H:i:s:A') }}
                                </td>

                                <td class="px-6 py-4">
                                    @php
                                        $dropdownId = 'dropdown-' . $podcast->id;
                                    @endphp

                                    <button id="toggle-{{ $podcast->id }}" data-dropdown-toggle="{{ $dropdownId }}"
                                    ...
                                    <div id="{{ $dropdownId }}"
                                        ...
                                        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200"
                                            ...
                                            <li>
                                                <x-button variant="link" target="delete-podcast-{{ $podcast->id }}">
                                                    <a href="#">Delete</a>
                                                </x-button>
                            ...
                            <x-alert-modal variant='delete' id="delete-podcast-{{ $podcast->id }}" title="Photograph"
                                :action="route('podcasts.destroy', ['podcast' => $podcast->id])" method="DELETE" />
                        @endforeach
