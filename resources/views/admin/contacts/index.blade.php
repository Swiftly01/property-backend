...
@section('content')
    ...
    <div class="p-4 sm:p-6 lg:p-10">
        ...
        <div class="flex flex-wrap items-center justify-between mt-16 md:mt-0">
            <h1 class="font-bold text-gray-900 md:text-2xl">All Contact</h1>
        ...
        <div class="mt-5 bg-white rounded lg:mt-14">
            <div class="flex flex-wrap items-center justify-between px-8 py-5">
                <h3 class="font-bold">All contact</h3>

                <form id="searchForm" action="{{ route('contacts.index') }}" method="GET">
                    <div class="flex flex-wrap items-center gap-2 md:gap-4">
                        ...
                        <x-filter :options="\App\Enums\InquiryEnum::cases()" name="status" />
                    ...
                    <tbody>
                        @foreach ($contacts as $contact)
                            <tr
                                ...
                                <td class="px-6 py-4">
                                    {{ Str::ucfirst($contact->firstname) }} {{ Str::ucfirst($contact->lastname) }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Str::ucfirst($contact->email)  }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $contact->phone_number }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Str::ucfirst($contact->inquiry_type ) }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <button id="toggle-{{ $contact->id }}" data-dropdown-toggle="{{ $dropdownId }}"
